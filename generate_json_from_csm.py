...
if __name__ == "__main__":
    ...
    parser.add_argument("-o", "--output", type=str, help="Output file name (optional). If not provided, JSON files will be created in the same directory as the CSM files.")
    args = parser.parse_args()

    # generate_json_from_csm(args.directory)
    try:
        chunkstore = Chunkstore(args.directory)
        data = chunkstore.csm_to_json()
        with open(args.optput, "w") as json_file:
