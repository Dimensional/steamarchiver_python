...
def read_workshop_file(workshop_file):
    """Read workshop IDs from JSON or CSV file generated by workshop_scraper"""
    workshop_ids = []
    file_ext = workshop_file.lower()
    
    if file_ext.endswith('.json'):
        with open(workshop_file, 'r', encoding='utf-8') as file:
            data = json.load(file)
            for item in data:
                if 'id' in item:
                    try:
                        workshop_ids.append(int(item['id']))
                    except ValueError:
                        print(f"Warning: Invalid workshop ID '{item['id']}' in {workshop_file}")
                        continue
        ...
        with open(workshop_file, 'r', encoding='utf-8') as file:
            ...
            for row in csv_reader:
                if 'id' in row:
                    try:
                        workshop_ids.append(int(row['id']))
                    except ValueError:
                        print(f"Warning: Invalid workshop ID '{row['id']}' in {workshop_file}")
                        continue
                else:
                    print(f"Warning: No 'id' column found in CSV row: {row}")
    else:
        raise ValueError(f"Unsupported file format. Expected .json or .csv, got: {workshop_file}")
...
if __name__ == "__main__": # exit before we import our shit if the args are wrong
    ...
    # Parse workshop IDs (handle comma-separated values and workshop files)
    workshop_ids = []
    ...
    # Handle workshop file input
    if args.workshop_file:
        try:
            file_workshop_ids = read_workshop_file(args.workshop_file)
            if args.workshop_ids:
                # Combine file IDs with command line IDs
                args.workshop_ids.extend(file_workshop_ids)
            else:
                args.workshop_ids = file_workshop_ids
            print(f"Loaded {len(file_workshop_ids)} workshop IDs from {args.workshop_file}")
        except Exception as e:
            print(f"Error reading workshop file {args.workshop_file}: {e}")
            exit(1)

    if not args.app_depot and not args.workshop_ids and not args.csv_file and not args.workshop_file:
        print("must specify at least one appid, workshop file id, workshop file, or CSV file")
        parser.print_help()
        exit(1)
